...
function update(delta) {
  ...
  if (Math.hypot(player.x - hazard.x, player.y - hazard.y) <= hazard.radius + player.radius && player.hurtTimer <= 0) {
    applyDamage(hazard.damage);
    player.hurtTimer = hazard.interval;
  }
...
function drawHazard(time) {
  ...
  ctx.strokeStyle = "rgba(239, 111, 143, 0.82)";
  ctx.lineWidth = 4;
  ctx.beginPath();
  ...
  ctx.strokeStyle = "rgba(246, 242, 233, 0.74)";
  ctx.lineWidth = 6;
  ctx.beginPath();
  ctx.moveTo(-24, 0);
  ctx.lineTo(24, 0);
...
function drawMinimap() {
  ...
  ctx.fillStyle = "rgba(239, 111, 143, 0.9)";
  ctx.beginPath();
  ctx.arc(x + hazard.x * scale, y + hazard.y * scale, 4, 0, Math.PI * 2);
  ctx.fill();
